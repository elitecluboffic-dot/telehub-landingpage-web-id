...
//    - PNG/JPG/WEBP -> watermark tiled, format sama seperti aslinya.
//    - GIF -> watermark tiled dari frame pertama saja, dikembalikan
//      sebagai PNG statis (bukan animasi, bukan file gif asli).
//    Ini supaya listing marketplace tetap bisa menampilkan gambar ke
//    calon pembeli, bukan kotak kosong / 403 total.
...
export async function handleAssetProxy(request, env, encodedFilename) {
  ...
  if (isOwner || isAdmin) {
    // Pemilik sah / admin -> file asli, full quality, format asli.
    headers.set("Content-Type", CONTENT_TYPES[ext]);
    return new Response(object.body, { headers });
  }

  // Non-owner (tapi tetap sudah login) -> preview ber-watermark.
  try {
    ...
    // Kalau proses watermark gagal (mis. logo watermark belum ada di R2),
    // jangan sampai bocorin file asli. Fallback ke forbidden.
    return new Response("Forbidden", { status: 403 });
  }

  // Format yang tidak dikenali sama sekali di atas -> forbidden.
  return new Response("Forbidden", { status: 403 });
}
